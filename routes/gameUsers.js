...
router.post('/', async function(req, res, next) {
    ...
    if(checktelephone.test(telephone)===false) return res.send({msg:'输入的手机号不符合规范!',resultCode:400}) 
    if(checkusername.test(username)===false) return res.send({msg:'输入的用户名不符合规范!',resultCode:400}) 
    ...
    if(model){
        ...
        var gameusers = await models.GameUser.create(req.body);
...
router.post('/login', async function(req, res, next) {
    ...
    const model = await models.GameUser.findOne({where:{username,password}})
    if(!model){
